...
// Spread Presets
// ==============================
// 캐릭터 이름 (NPC/PC 대체에 사용)
let charNames = { a: "NPC", b: "PC" };
...
const SPREAD_PRESETS = [
    {
        ...
        defaultQuestion: "{{char}}와 {{user}}의 관계와 서로의 현재 마음에 대한 점"
    },
    {
        ...
        defaultQuestion: "{{char}}에 대한 과거, 현재, 미래에 대한 점"
    },
    {
        ...
        defaultQuestion: "{{char}}와 {{user}} 사이의 문제점과 대처, 미래 흐름"
    },
    {
        ...
        defaultQuestion: "{{char}}와 {{user}} 사이의 갈등 상황에 대한 발생 배경과 현재, 대처 방법"
    },
    {
        id: "mind-heart-path",
        label: "마음 / 감정 / 길",
        slots: ["마음 (이성)", "감정", "나아갈 길"],
        keys:  ["mind", "heart", "path"],
        defaultQuestion: "{{char}}의 마음과 감정의 흐름은?"
    },
    {
        id: "custom",
        label: "✏️ 직접 입력",
...
// custom 슬롯 상태 (가변)
let customSlotList = ["슬롯 1", "슬롯 2", "슬롯 3"];
let customQuestion = "";

function applyCharNames(str) {
    return (str || "")
        .replace(/\{\{char\}\}/g, charNames.a)
        .replace(/\{\{user\}\}/g, charNames.b);
}

function buildOoc(spread, slots, question) {
    const keys = slots.map((_, i) => spread.id === "custom" ? `slot${i+1}` : (spread.keys[i] || `slot${i+1}`));
    const resolvedSlots = slots.map(s => applyCharNames(s));
    const resolvedQ = applyCharNames(question || "");
    const slotDesc = resolvedSlots.join("/");
    const spreadLine = applyCharNames(spread.id === "custom" ? slotDesc : spread.label);

    const firstKey = keys[0];
    const firstBlock = `"${firstKey}": {\n"name": "카드 이름",\n"theme": "테마 컬러(hex)",\n"number": "숫자 또는 suit:rankIdx",\n"direction": "up/down",\n"interpretation": "해석",\n"comment": "${charNames.a} 코멘트",\n"userComment": "${charNames.b} 코멘트"\n}`;
    const restBlocks = keys.slice(1).map(key => `"${key}": {\n(동일 양식)\n}`).join(",\n");
    const json = restBlocks ? `{\n${firstBlock},\n${restBlocks}\n}` : `{\n${firstBlock}\n}`;

    const questionLine = resolvedQ ? `\n질문: ${resolvedQ}` : "";
    return `ooc : 잠시 롤플 중지. 지정 양식만 제출할 것.\n타로 스프레드: ${spreadLine} (${slotDesc})${questionLine}\n카드를 ${slots.length}장 뽑는다.\n- 출력 양식 (json)\n\`\`\`\n${json}\n\`\`\``;
}
...
function getCardImgPath(raw) {
    ...
    if (parsed.type === "minor") {
        ...
        return card ? `cards/m_${card.imgId}.png` : "";
    }
